"""L5K Parser v2 — clean line-by-line dispatch, no pos hacking."""
import re
...
def parse_l5k(text_or_path: str) -> ControllerDef:
    ...
    while i < len(lines):
        ...
        if line.startswith("IE_VER"):
            m = re.search(r"IE_VER\s*:=\s*(\S+)", line)
            if m:
                controller.ie_ver = m.group(1).strip(";")

        elif line.startswith("CONTROLLER "):
            m = re.match(r"CONTROLLER\s+(\S+)\s*\((.*)\)", line, re.DOTALL)
            if m:
...
def _parse_tag_block(lines: list[str], i: int, scope: str):
    ...
    while i < len(lines):
        ...
        m = re.match(r'(\w+(?:\[\d+\.\.\d+\])?)\s+(\w+)\s*(.*)', stripped)
        if m:
            name = m.group(1)
            dtype = m.group(2)
            rest = m.group(3)
...
def _parse_program(lines: list[str], i: int):
    ...
    while i < len(lines):
        ...
        if line == "END_PROGRAM":
            ...
            rm = re.match(r"ROUTINE\s+(\S+)\s*(?:\(.*Type\s*:=\s*(\w+))?", line)
            if rm:
                prog.routines.append(RoutineDef(
                    name=rm.group(1),
                    language=rm.group(2) or "Ladder"
                ))
